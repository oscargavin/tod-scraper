#!/usr/bin/env python3
"""
Boots Review Enricher
Processes Which.com product JSON files and enriches them with Boots review data
"""
import asyncio
import json
import re
...
from src.reviews.boots.search import search_and_extract
from src.reviews.boots.sentiment_scraper import get_sentiment_analysis
...
def extract_search_terms(product_name: str, category: str = None) -> tuple[str, Optional[str]]:
    ...
    Args:
        ...
        category: Category slug (e.g., 'air-fryers', 'washing-machines')
    """
    ...
    if category:
        # Map category slugs to Boots search terms
        category_mappings = {
...
async def process_product(product: Dict, index: int, total: int, page=None, worker_id: int = 0,
    ...
    """
    Process a single product and add Boots review data
    ...
    try:
        ...
        if result['success']:
            ...
            if extract_sentiment and boots_url:
                try:
                    # Get streamlined sentiment analysis
                    sentiment = await get_sentiment_analysis(boots_url, max_pages=4)
            ...
            # Add Boots reviews to product (separate from AO reviews if present)
            if 'reviews' not in product:
                product['reviews'] = reviews_data
            else:
                # If AO reviews already exist, add Boots as alternative
                product['bootsReviews'] = reviews_data
...
async def process_file(input_file: str, output_file: str = None, limit: int = None, workers: int = 3,
    ...
    """
    Process a JSON file of products and enrich with Boots reviews
    ...
    if use_parallel:
        ...
        async with async_playwright() as p:
            ...
            if not category and input_path.stem:
                ...
                if 'air-fryer' in filename or 'airfryer' in filename:
                    category = 'air fryer'
                elif 'coffee' in filename:
                    category = 'coffee machine'
                elif 'kettle' in filename:
                    category = 'kettle'
                else:
                    category = 'appliance'
    ...
    # Save output
    if not output_file:
        suffix = '_boots_reviews_sentiment' if extract_sentiment else '_boots_reviews'
    ...
    # Print summary
    print("\n" + "="*60)
    print("BOOTS REVIEW ENRICHMENT SUMMARY")
    print("="*60)
...
def main():
    parser = argparse.ArgumentParser(
        description='Enrich Which.com product data with Boots reviews',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Process air fryers file
  python enricher.py output/air-fryers_full.json

  # Process with custom output file
  python enricher.py output/coffee-machines.json -o enriched_coffee.json

  # Process only first 10 products (for testing)
  python enricher.py output/kettles.json --limit 10

  # Process with sentiment analysis
  python enricher.py output/air-fryers_full.json --sentiment
        """
    )
    ...
    parser.add_argument('-o', '--output',
                       help='Output file name (default: input_boots_reviews.json)')
    ...
    parser.add_argument('-c', '--category',
                       help='Product category for sentiment context (e.g. "air fryer", "kettle")')
